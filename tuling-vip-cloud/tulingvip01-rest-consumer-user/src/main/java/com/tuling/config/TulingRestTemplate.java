...
public class TulingRestTemplate extends RestTemplate {
     ...
     * @return
     */
    private static   URI replaceUrl(URI url){
        ...
        List<String> ipList = MainStore.storeMap.get(serviceName);
        Random random = new Random();
        Integer randomIndex = random.nextInt(ipList.size());
        log.info("随机下标:{}",randomIndex);
        String serviceIp = ipList.get(randomIndex);
        log.info("随机选举的服务IP:{}",serviceIp);
        String targetSource = httpUrl[0]+"//"+httpUrl[1].replace(serviceName,serviceIp);
        try {
